...
>
> **Follow-up — authentication audit + new sign-in methods (built):** audited all three identity planes (platform-operator, merchant + roles, partner) — see `AUTH_AUDIT.md`. Added to the merchant `User` plane: **email + password** (scrypt), **phone-number OTP** (hashed, TTL + attempt-lockout), **Google/Apple OAuth** (tokeninfo / Apple-JWKS verification, injectable for tests), and **TOTP two-factor** (encrypted secret, challenge/verify at login). REST under `/auth/*`; admin LoginGate (password/phone/2FA) + Settings security panel. Documented follow-ups: 2FA for operators, merchant magic-link enumeration parity, OTP edge rate-limiting. Tests **218 core / 11 api / 4 mcp** green.
